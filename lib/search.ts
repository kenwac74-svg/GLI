...
export function extractCriteria(query: string): SearchCriteria {
  ...
  const seasonal = /별장|세컨드\s*하우스|겨울마다|개월.*(?:쉬|놀|체류)|내가 없을 때/.test(text);
  const explicitRent = /임대|월세|렌트|rent/.test(text);
  const explicitSale = /매매|구매|매입|sale/.test(text);
  const type = /빌라|villa/.test(text) ? "villa" : /주택|하우스|house/.test(text) ? "house" : /콘도|아파트|condo|apartment/.test(text) ? "condo" : null;
  ...
  if (krwMatch) {
    const unit = krwMatch[2] === "억" ? 100_000_000 : krwMatch[2] === "천만" ? 10_000_000 : 10_000;
...
export function searchAssets(query: string, allAssets: Asset[]): SearchResult {
  ...
  const exact = allAssets.filter((asset) => {
    ...
    if (criteria.wantsRiver && !/river|mekong|riverside|강|메콩/i.test(`${asset.title} ${asset.summary}`)) return false;
    return true;
  });
  const pool = exact.length ? exact : allAssets.filter((asset) =>
...
function buildReasons(asset: Asset, criteria: SearchCriteria, nearest: boolean): string[] {
  ...
  if (criteria.bedrooms !== null && asset.bedrooms === criteria.bedrooms) reasons.push(`${asset.bedrooms}베드 조건`);
...
function buildClarification(criteria: SearchCriteria): string | null {
  if (criteria.purpose === "seasonal" && criteria.maxPriceUsd === null) return "매입 예산 상한과 머무를 인원을 알려주시면 후보를 더 정확히 좁힐 수 있습니다.";
  if (criteria.purpose === "income" && criteria.maxPriceUsd === null) return "총 매입 예산과 목표 월 임대료가 있나요?";
  if (criteria.transaction === null) return "매매와 임대 중 어느 쪽을 우선으로 볼까요?";
  return null;
}
...
function buildAnswer(criteria: SearchCriteria, exactCount: number, shownCount: number): string {
  ...
  if (criteria.purpose === "seasonal") return `3개월 체류와 부재 중 운영을 함께 고려한 후보 ${shownCount}개를 정리했습니다. 단기 임대 가능 여부는 건물 규정, 현지 규제와 운영대행 수수료를 별도로 확인해야 합니다.`;
